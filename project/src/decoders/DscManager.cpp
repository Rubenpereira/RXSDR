...
DscManager::DscManager(QObject* parent) : QObject(parent)
{
    aplicarParams();
}
...
void DscManager::setParams(const Params& p)
{
    params_ = p;
    aplicarParams();
}

void DscManager::aplicarParams()
{
    DscCore::Params cp;
...
QString DscManager::stateString() const
{
    switch (state_) {
...
QJsonObject DscManager::statusJson() const
{
    QJsonObject o;
    o["state"]      = stateString();
    ...
    o["symbols"]    = core_.simbolos();
    o["valid"]      = core_.validos();
    o["fecFixes"]   = core_.corrigidos();
    o["messages"]   = core_.mensagens();
    if (!lastError_.isEmpty()) o["error"] = lastError_;
    return o;
}

bool DscManager::start()
{
    ...
    emit logLine(QStringLiteral("[DSC] decodificador iniciado - %1 baud, shift %2 Hz, tom central %3 Hz")
                     .arg(double(params_.baudRate)).arg(double(params_.shift)).arg(double(params_.centerFreq)));
    return true;
}

void DscManager::stop()
{
    if (state_ == State::Stopped) return;

    if (!linhaParcial_.trimmed().isEmpty())
        emit logLine(QStringLiteral("[DSC] ") + linhaParcial_.trimmed());
    linhaParcial_.clear();

    state_ = State::Stopped;
    emit stateChanged(state_);
    emit logLine(QStringLiteral("[DSC] decodificador parado"));
}
...
// ---------------------------------------------------------------------------
void DscManager::feedAudio(const int16_t* samples, int count, uint32_t sps)
{
...
void DscManager::despejarTexto(const std::string& txt)
{
    for (char c : txt) {
        if (c == '\n' || c == '\r') {
            const QString l = linhaParcial_.trimmed();
            if (!l.isEmpty()) emit logLine(QStringLiteral("[DSC] ") + l);
            linhaParcial_.clear();
        } else {
            linhaParcial_ += QLatin1Char(c);
            if (linhaParcial_.size() >= 200) {
                emit logLine(QStringLiteral("[DSC] ") + linhaParcial_);
                linhaParcial_.clear();
            }
        }
    }
